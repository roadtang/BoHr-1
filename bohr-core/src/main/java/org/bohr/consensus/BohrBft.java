 ...
 */
package org.bohr.consensus;

import static org.bohr.core.Fork.UNIFORM_DISTRIBUTION;
...
import org.bohr.consensus.BohrBft.Event.Type;
import org.bohr.consensus.exception.BohrBftException;
...
import org.bohr.core.TransactionExecutor;
...
import org.bohr.crypto.Key.Signature;
...
import org.bohr.net.msg.consensus.NewViewMessage;
import org.bohr.net.msg.consensus.ProposalMessage;
import org.bohr.net.msg.consensus.VoteMessage;
import org.bohr.util.ArrayUtil;
import org.bohr.util.ByteArray;
import org.bohr.util.Bytes;
import org.bohr.util.MerkleUtil;
...
/**
 * Implements Bohr BFT engine based on single-thread event model. States are
 ...
 * </ul>
 */
public class BohrBft implements BftManager {
    private static final Logger logger = LoggerFactory.getLogger(BohrBft.class);
    ...
    public BohrBft(Kernel kernel) {
        this.kernel = kernel;
        this.config = kernel.getConfig();

        this.chain = kernel.getBlockchain();
        this.blockStore = new BohrBlockStore(chain);
    ...
    public void start() {
        if (status == Status.STOPPED) {
            ...
            timer.start();
            broadcaster.start();
            logger.info("Bohr BFT manager started");

            enterNewHeight();
            eventLoop();

            logger.info("Bohr BFT manager stopped");
        }
    }
    ...
    protected void enterCommit() {
        ...
        if (!blockHash.isPresent()) {
            throw new BohrBftException("Entered COMMIT state without +2/3 pre-commit votes");
        } else {
    ...
    protected Block proposeBlock() {
        ...
        BohrBlock bohrBlock = new BohrBlock(tempHeader, config.spec().maxBlockGasLimit());
        ...
        for (PendingManager.PendingTransaction pendingTx : pendingTxs) {
            ...
            TransactionResult result = exec.execute(tx, asTrack, dsTrack, bohrBlock, gasUsedInBlock);
    ...
    protected boolean validateBlockProposal(BlockHeader header, List<Transaction> transactions) {
        try {
            ...
            List<TransactionResult> results = transactionExecutor.execute(transactions, asTrack, dsTrack,
                    new BohrBlock(header, config.spec().maxBlockGasLimit()), 0);
