...
import org.bohr.core.BlockchainImpl;
...
import org.bohr.util.ConsoleUtil;
...
/**
 * Bohr command line interface.
 */
public class BohrCli extends Launcher {

    public static final boolean ENABLE_HD_WALLET_BY_DEFAULT = false;

    private static final Logger logger = LoggerFactory.getLogger(BohrCli.class);

    public static void main(String[] args, BohrCli cli) {
        try {
            // check jvm version
    ...
    public BohrCli() {
        ...
        Option changePasswordOption = Option.builder()
                .longOpt(BohrOption.CHANGE_PASSWORD.toString()).desc(CliMessages.get("ChangeWalletPassword")).build();
        addOption(changePasswordOption);

        Option dumpPrivateKeyOption = Option.builder()
                .longOpt(BohrOption.DUMP_PRIVATE_KEY.toString())
        ...
        Option importPrivateKeyOption = Option.builder()
                .longOpt(BohrOption.IMPORT_PRIVATE_KEY.toString())
        ...
        Option reindexOption = Option.builder()
                .longOpt(BohrOption.REINDEX.toString())
    ...
    public void start(String[] args) throws ParseException, IOException {
        ...
        if (cmd.hasOption(BohrOption.HELP.toString())) {
            printHelp();

        } else if (cmd.hasOption(BohrOption.VERSION.toString())) {
            printVersion();

        } else if (cmd.hasOption(BohrOption.ACCOUNT.toString())) {
            String action = cmd.getOptionValue(BohrOption.ACCOUNT.toString()).trim();
        ...
        } else if (cmd.hasOption(BohrOption.CHANGE_PASSWORD.toString())) {
            changePassword();

        } else if (cmd.hasOption(BohrOption.DUMP_PRIVATE_KEY.toString())) {
            dumpPrivateKey(cmd.getOptionValue(BohrOption.DUMP_PRIVATE_KEY.toString()).trim());

        } else if (cmd.hasOption(BohrOption.IMPORT_PRIVATE_KEY.toString())) {
            importPrivateKey(cmd.getOptionValue(BohrOption.IMPORT_PRIVATE_KEY.toString()).trim());

        } else if (cmd.hasOption(BohrOption.REINDEX.toString())) {
            reindex(cmd.getOptionValue(BohrOption.REINDEX.toString()));
    ...
    protected void printHelp() {
        ...
        formatter.printHelp("./bohr-cli.sh [options]", getOptions());
    }
