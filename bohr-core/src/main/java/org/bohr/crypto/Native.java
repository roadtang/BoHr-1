...
/**
 * Bohr crypto native implementation
 */
public class Native {
    ...
    protected static void init() {
        ...
        case LINUX:
            if (SystemUtil.getOsArch().equals("aarch64")) {
                enabled = loadLibrary("/native/Linux-aarch64/libbohrcrypto.so");
            } else {
                enabled = loadLibrary("/native/Linux-x86_64/libbohrcrypto.so");
            }
            break;
        case MACOS:
            enabled = loadLibrary("/native/Darwin-x86_64/libbohrcrypto.dylib");
            break;
        case WINDOWS:
            enabled = loadLibrary("/native/Windows-x86_64/libbohrcrypto.dll");
            break;
        default:
            break;
        }
