...
import org.bohr.consensus.VoteType;
import org.bohr.core.Genesis.Premine;
import org.bohr.core.event.BlockchainDatabaseUpgradingEvent;
import org.bohr.core.exception.BlockchainException;
import org.bohr.core.state.*;
...
public class BlockchainImpl implements Blockchain {
    ...
    private final BlockStore blockStore = new BohrBlockStore(this);
    ...
    @Override
    public Transaction getRewardTransaction(long blockNumber) {
        if (blockNumber == 0) {
            return null;
        }

        byte[] b = indexDB.get(Bytes.merge(TYPE_BLOCK_COINBASE_REWARD_BY_NUMBER, Bytes.of(blockNumber)));
        if (b == null) {
            return null;
        }
    ...
    public synchronized void addBlock(Block block) {
        ...
        for (int i = 0; i < txs.size(); i++) {
            ...
            for (BohrInternalTransaction internalTx : result.getInternalTransactions()) {
        ...
        if (number != genesis.getNumber()) {
            // [4] coinbase fee reward transaction
    ...
    @Override
    public List<BohrInternalTransaction> getInternalTransactions(byte[] address, int from, int to) {
        List<BohrInternalTransaction> list = new ArrayList<>();

        int total = getInternalTransactionCount(address);
        for (int i = from; i < total && i < to; i++) {
            byte[] key = getNthInternalTransactionIndexKey(address, i);
            byte[] value = indexDB.get(key);
            list.add(BohrInternalTransaction.fromBytes(value));
        }
     ...
     * @param address
     */
    protected void addInternalTransactionToAccount(Transaction root, BohrInternalTransaction tx, byte[] address) {
        int total = getInternalTransactionCount(address);
     ...
     * @return
     */
    protected boolean validateBlock(Block block, AccountState asTrack, DelegateState dsTrack,
        ...
        try {
            ...
            TransactionExecutor transactionExecutor = new TransactionExecutor(config, blockStore,isVMEnabled(),isVotingPrecompiledUpgraded());
            List<TransactionResult> results = transactionExecutor.execute(transactions, asTrack, dsTrack,
                    new BohrBlock(block.getHeader(), config.spec().maxBlockGasLimit()),0);
