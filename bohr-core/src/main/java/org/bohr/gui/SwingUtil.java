...
import static org.bohr.gui.TextContextMenuItem.COPY;
import static org.bohr.gui.TextContextMenuItem.CUT;
import static org.bohr.gui.TextContextMenuItem.PASTE;
...
public class SwingUtil {
	...
	public static Unit defaultUnit = BOHR;
	...
	public static void setColumnAlignments(JTable table, Integer... right) {
		TableColumnModel model = table.getColumnModel();
		for (int i = 0; i < right.length; i++) {
			DefaultTableCellRenderer rightRenderer = new DefaultTableCellRenderer();
			rightRenderer.setHorizontalAlignment(right[i]);
			rightRenderer.setBorder(new EmptyBorder(0, 0, 0, 0));
			model.getColumn(i).setCellRenderer(rightRenderer);
		}
	}
	...
	public static JTextField textFieldWithCopyPastePopup(JTextField textField) {
		textField.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 13));
		addTextContextMenu(textField, Arrays.asList(COPY, PASTE, CUT));
		addTextMouseClickFocusListener(textField);
		return textField;
	...
	/**
	 * Formats a Bohr value.
	 *
	 * @param a
	 * @return
	 */
	public static String formatAmount(Amount a) {
		return formatAmount(a, BOHR, fractionDigits, true);
	}
	...
	public static String formatAmountNoUnit(Amount a) {
		return formatAmount(a, BOHR, fractionDigits, false);
	}

	/**
	 * Formats a Bohr value without truncation.
	 *
	 * @param a
	 * @return
	 */
	public static String formatAmountFull(Amount a) {
		return formatAmount(a, BOHR, 9, true);
	}
	...
	private static String formatAmount(Amount a, Unit unit, int fractionDigits, boolean withUnit) {
		return formatNumber(a.toDecimal(fractionDigits, unit), fractionDigits) + (withUnit ? (" " + unit.symbol) : "");
	}
	...
	public static Amount parseAmount(String str) throws ParseException {
		Unit unit = stream(Unit.values()).filter(u -> str.endsWith(" " + u.symbol)).findAny()
				.orElse(BOHR);
	 ...
	 * @return
	 */
	public static String getTransactionDescription(BohrGui gui, Transaction tx) {
		switch (tx.getType()) {
		case COINBASE:
	 ...
	 * @return
	 */
	public static String describeAddress(BohrGui gui, byte[] address) {
		return getAddressAlias(gui, address)
				.orElse(getAddressDelegateName(gui, address).orElse(getAddressAbbr(address,8)));
	}
	 ...
	 * @return
	 */
	public static Optional<String> getAddressAlias(BohrGui gui, byte[] address) {
	 ...
	 * @return
	 */
	public static Optional<String> getAddressDelegateName(BohrGui gui, byte[] address) {
	 ...
	 * @return
	 */
	public static String getAddressAbbr(byte[] address,int len) {
		...
		String fullAddress = Hex.encode(address);
		String newAddress = org.bohr.util.StringUtil.hexToBase58(fullAddress);
		return newAddress.substring(0,len)+ "..."+newAddress.substring(newAddress.length()-len,newAddress.length());
	}


	public static String getAddressAbbr_new(byte[] address, int len) {
		//StringBuilder sb = new StringBuilder();
		//sb.append(Hex.PREF);
		//sb.append(Hex.encode(Arrays.copyOfRange(address, 0, len)));
		//sb.append("...");
		//sb.append(Hex.encode(Arrays.copyOfRange(address, address.length - len, address.length)));
		//return sb.toString();
	 ...
	 * @return
	 */
	public static String getAddressAbbr_hex(byte[] address) {
		return Hex.PREF + Hex.encode(Arrays.copyOfRange(address, 0, 2)) + "..."
				+ Hex.encode(Arrays.copyOfRange(address, address.length - 2, address.length));
	}

	public static String getAddressAbbr_new_hex(byte[] address) {
		return getAddressAbbr_new(address, 5);
	}

	public static String getAddressAbbr_new_hex(byte[] address, int len) {
		StringBuilder sb = new StringBuilder();
		sb.append(Hex.PREF);
		sb.append(Hex.encode(Arrays.copyOfRange(address, 0, len)));
		sb.append("...");
