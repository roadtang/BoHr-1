...
public class WalletModel {
	 ...
	 *
	 * @param syncProgress Value to set for property ${@link #syncProgress}.
	 */
	 ...
	 *
	 * @param validators Value to set for property 'validators'.
	 */
	...
	public Optional<String> getValidator(int view) {
		...
		return Optional.ofNullable(config.spec().getPrimaryValidator(validators, latestBlock.getNumber() + 1, view,
	...
	public Optional<WalletDelegate> getValidatorDelegate(int view) {
		...
		return delegates.stream().filter(wd -> wd.getAddressString().equals(getValidator(view).get())).findFirst();
	}
	...
	public Optional<WalletDelegate> getNextPrimaryValidatorDelegate() {
		...
		return delegates.stream().filter(wd -> wd.getAddressString()
				.equals(validators.get((int) ((latestBlock.getNumber() + 2) % validators.size())))).findFirst();
	}
	...
	public Optional<Long> getNextValidatorSetUpdate() {
		...
		return Optional.of(((latestBlock.getNumber() + 1) / config.spec().getValidatorUpdateInterval() + 1)
	...
	public void chooseView(Action action, String clsName) {
		for (ActionListener listener : listeners) {
			if (listener.getClass().getSimpleName().equals(clsName)) {
				EventQueue.invokeLater(() -> listener.actionPerformed(new ActionEvent(this, 0, action.name())));
			}
		}
	}
